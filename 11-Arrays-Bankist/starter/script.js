...
arr.splice(-1);
console.log(arr);
arr.splice(1, 2) // 1번 index에서 2개의 값을 제거
console.log(arr);
...
const displayMovements = function(movements, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    // 'beforebegin' : 타켓 요소 전(형제레벨)에 생성- 시작 태그의 앞(형제 레벨로),
    // 'afterbegin' : 타켓 요소 다음(자식요소)에 생성 - 시작 태그의 뒤(자식 요소로),
    // 'beforeend' : 타켓 요소 '끝나는 태그' 바로 직전(자식요소로)에 요소를 생성 - '종료 태그' 앞(자식 요소로),
    // 'afterend' : 타켓 요소의 '끝나는 태그' 바로 다음(형제레벨)에 요소를 생성 - '종료 태그' 뒤(형제 레벨로)
  });
};
